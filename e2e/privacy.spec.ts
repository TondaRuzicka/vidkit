...
for (const file of ['e2e-10s.mp4', 'e2e-4s.avi']) {
  test(`no request leaves the origin during a ${file} compress (incl. engine load)`, async ({
    ...
    // Cookieless analytics: pageview beacons only, never file data. Exempt from
    // the origin/body checks; every other host stays strictly forbidden.
    const ANALYTICS_HOSTS = ['cloud.umami.is', 'gateway.umami.is'];
    const offenders: string[] = [];
    page.on('request', (req) => {
      const url = new URL(req.url());
      if (ANALYTICS_HOSTS.includes(url.host)) return;
